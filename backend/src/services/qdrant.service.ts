...
export async function searchAssessments(query: string, limit: number = 5): Promise<SHLAssessment[]> {
    try {
        ...
        let vector: number[];
        if (expandedOutput) {
            const origVec = Array.from(origOutput.data) as number[];
            const expVec = Array.from(expandedOutput.data) as number[];
            ...
            vector = Array.from(origOutput.data) as number[];
        }
        ...
        const unique = new Map<string, SHLAssessment & { _score: number }>();
