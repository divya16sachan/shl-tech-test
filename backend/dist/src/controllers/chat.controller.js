...
function sendStatus(res, status, detail) {
    res.write(`data: ${JSON.stringify({ type: 'status', status, detail })}\n\n`);
}
function sendEvent(res, payload) {
    res.write(`data: ${JSON.stringify(payload)}\n\n`);
}
async function classifyIntent(userMessage, memorySummary, model) {
...
function buildSystemPrompt(intent, memorySummary, assessments) {
    const catalogBlock = assessments.length > 0
...
export async function handleChat(req, res) {
    const { conversationId, messages, model = 'llama-3.3-70b-versatile', } = req.body;
    ...
    try {
        ...
        const intent = await classifyIntent(userMessage, conversation.memorySummary, model);
        ...
        const systemPrompt = buildSystemPrompt(intent, conversation.memorySummary, assessments);
        ...
        updateMemorySummary(conversation.memorySummary, userMessage, fullReply).then(async (newSummary) => {
