...
// ─── Scoring ──────────────────────────────────────────────────────────────────
export function recallAtK(retrieved, expected, k = 10) {
    if (expected.length === 0)
        return 1.0;
    const topK = retrieved.slice(0, k);
    const hits = expected.filter((e) => topK.some((name) => name.toLowerCase().includes(e.toLowerCase()) ||
        e.toLowerCase().includes(name.toLowerCase()))).length;
    return hits / expected.length;
}
export function isGrounded(assessments) {
...
export function buildNote(def, retrieved, recall, intent) {
    if (def.isVague) {
